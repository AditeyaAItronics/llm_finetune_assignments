...
(function () {
  ...
  function compute() {
    ...
    // Per the assignment, only English (X1) carries the <=1.2 requirement.
    var gate = rows["en"].X <= GATE;
    return {
  ...
  function render() {
    ...
    // score card — score is defined regardless of the gate; always show it
    $("score").textContent = r.score === Infinity ? "∞" : Math.round(r.score).toLocaleString();
    $("spread").textContent = fmt(r.spread);
    var badge = $("gate");
    badge.textContent = r.gate ? "English X₁ ≤ 1.2 ✓" : "English X₁ = " + fmt(r.rows["en"].X, 3) + " (> 1.2)";
    ...
    c += "score  S = 1000 / Δ = <b>" + (r.score === Infinity ? "∞" : fmt(r.score, 1)) + "</b>";
    c += r.gate ? "" : "\n(note: only English X₁ must be ≤ 1.2 — it is " + fmt(r.rows["en"].X, 3) + " here)";
    $("calc").innerHTML = c;
